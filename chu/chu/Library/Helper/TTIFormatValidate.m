...
//正整数
+ (BOOL) validateIntegerNum:(NSString *)number
{
    NSString *passWordRegex = @"^[0-9]*[1-9][0-9]*$";
    NSPredicate *passWordPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",passWordRegex];
    return [passWordPredicate evaluateWithObject:number];
}
//小数点不能多于后两位
+ (BOOL) validateFloatTwoWithStr:(NSString *)floatStr
{
    NSString *passWordRegex = @"^\\d+(?:\\.\\d{0,2})?$";
    NSPredicate *passWordPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",passWordRegex];
    return [passWordPredicate evaluateWithObject:floatStr];
}
//水电气缴费账户验证
+ (BOOL) validateConvenienceWithStr:(NSString *)accountStr
{
    NSString *passWordRegex = @"^\\d{7,10}$";
    NSPredicate *passWordPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",passWordRegex];
    return [passWordPredicate evaluateWithObject:accountStr];
}
